...
from analysis_service.irt.estimation.config import (
    EstimationConfig,
)
...
class ExpectedAbilityEstimates:
    """
...
def _calculate_ability_posterior(
    data: ResponseMatrix,
...
def sample_abilities(
    ...
    """
    Draw a sample from the posterior of the ability distribution
    """
    if config is None:
    ...
    posteriors = _calculate_ability_posterior(
        data=data, model=model, quadrature=quadrature
    )

    # Sample one theta value for each candidate according to their posterior
    sampled_indices = np.array(
        [rng.choice(n_quad, p=posteriors[i, :]) for i in range(n_candidates)]
    )

    sampled_theta = theta[sampled_indices]
    assert isinstance(sampled_theta, np.ndarray)

    return sampled_theta


def estimate_abilities_eap(
    data: ResponseMatrix,
    model: IRTEstimationResult,
    config: EstimationConfig | None = None,
) -> ExpectedAbilityEstimates:
    """
    ...
    posteriors = _calculate_ability_posterior(data, model, quadrature)
    ...
    return ExpectedAbilityEstimates(eap=eap, se=se)
...
def _compute_item_log_likelihood_for_abilities(
    ...
    Args:
        ...
        params: Item parameters (K+1 categories including missing).
